...
ms.locfileid: "137819363"
---
# <a name="create-an-azure-cognitive-search-solution"></a>创建 Azure 认知搜索解决方案
...
如果订阅中还没有认知服务资源，则需要预配认知服务资源。 搜索解决方案将使用该资源通过 AI 生成的见解来丰富数据存储中的数据。
...
1. 在 Azure 门户中，浏览到 Azure 认知搜索资源。 然后，在其“概述”页上，选择“导入数据” 。
...
12. 将索引器名称改为 margies-indexer。 
13. 将“计划”设置为“一次” 。
14. 展开“高级”选项，并确保已选择“基础 64 编码密钥”选项（通常编码密钥会使索引更有效） 。
15. 选择“提交”以创建数据源、技能组、索引和索引器。 索引器将自动运行并运行索引管道，该索引管道可以：
    1. 从数据源中提取文档元数据字段和内容
...
## <a name="explore-and-modify-definitions-of-search-components"></a>浏览并修改搜索组件的定义

搜索解决方案的组件基于 JSON 定义，你可以在 Azure 门户中查看和编辑它。

你可以使用该门户创建和修改搜索解决方案，但理想的做法通常是用 JSON 定义搜索对象，并使用 Azure 认知服务 REST 接口来创建和修改它们。

### <a name="get-the-endpoint-and-key-for-your-azure-cognitive-search-resource"></a>获取 Azure 认知搜索资源的终结点和密钥
...
### <a name="review-and-modify-the-skillset"></a>评价和修改技能组
...
### <a name="review-and-modify-the-index"></a>查看并修改索引
...
### <a name="review-and-modify-the-indexer"></a>查看并修改索引器
...
4. 查看 ouputFieldMappings 部分，它将技能组中技能的数据映射到索引字段。 其中大部分结果都反映了你在用户界面中的选择，但还添加了以下映射，将由情绪识别技能提取的 sentimentLabel 值映射到你添加到索引的 sentiment 字段 ：
...
### <a name="use-the-rest-api-to-update-the-search-solution"></a>使用 REST API 更新搜索解决方案
...
### <a name="query-the-modified-index"></a>查询经修改的索引
...
## <a name="create-a-search-client-application"></a>创建搜索客户端应用程序
...
### <a name="get-the-endpoint-and-keys-for-your-search-resource"></a>获取搜索资源的终结点和密钥
...
### <a name="prepare-to-use-the-azure-cognitive-search-sdk"></a>准备使用 Azure 认知搜索 SDK
...
### <a name="explore-code-to-search-an-index"></a>浏览用于搜索索引的代码
...
### <a name="explore-code-to-render-search-results"></a>浏览用于呈现搜索结果的代码
